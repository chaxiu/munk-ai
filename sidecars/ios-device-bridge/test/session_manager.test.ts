...
import {
  AppiumPreinstalledWdaLauncher,
  IOSDeviceBridgeSessionManager,
  type SessionInfo,
  buildWdaCreateSessionPayload,
  listRealDevices,
  WdaBackend,
} from '../src/session_manager.js';
...
test('createSession writes diagnostics files and redacts sensitive fields', async () => {
  ...
  const summary = JSON.parse(
    fs.readFileSync(path.join(runDir, 'ios_bridge', 'summary.json'), 'utf8'),
  );
...
test('ensureWdaReady falls back to launch when status probe is a false positive', async () => {
  ...
  (backend as any).ensureClient = async () => client;
...
test('AppiumPreinstalledWdaLauncher retries once after Code=41 and then succeeds', async () => {
  ...
  let ensureCalls = 0;
  let closeCalls = 0;
  const launcher = new AppiumPreinstalledWdaLauncher(
    info,
    (_sessionInfo, _localPort) => ({
    ...
    async ensureReady() {
      ...
      if (ensureCalls === 1) {
        throw new IOSDeviceBridgeError(
          ...
          'Error Domain=XCTDaemonErrorDomain Code=41 "Not authorized for performing UI testing actions."',
          502,
          {
            response_payload: {
              value: {
  ...
  await launcher.launchAndWaitReady(client as any, {
    localPort: 8100,
    remotePort: 8100,
    baseUrl: 'http://127.0.0.1:8100',
  });
...
test('AppiumPreinstalledWdaLauncher returns actionable unlock error after repeated Code=41', async () => {
  ...
  const code41 = new IOSDeviceBridgeError(
    'wda_ui_testing_not_authorized',
    'Error Domain=XCTDaemonErrorDomain Code=41 "Not authorized for performing UI testing actions."',
    502,
    {
