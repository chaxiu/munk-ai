...
import { logBridgeEvent, normalizeForDiagnostics, withBridgeSpan, } from './diagnostics.js';
import { IOSDeviceBridgeError } from './errors.js';
import { DEFAULT_WDA_PORT, DEFAULT_WDA_REMOTE_PORT, } from './wda_connection.js';
...
export class AppiumPreinstalledWdaLauncher {
    info;
    wdaFactory;
    delayFn;
    wda = null;
    constructor(info, wdaFactory = createAppiumPreinstalledWda, delayFn = delay) {
        this.info = info;
        this.wdaFactory = wdaFactory;
        this.delayFn = delayFn;
    }
    async launchAndWaitReady(client, connection) {
        ...
        for (let attempt = 1; attempt <= WDA_UI_TESTING_AUTHORIZATION_MAX_ATTEMPTS; attempt += 1) {
            try {
                await withBridgeSpan(this.info.sessionId, 'bridge.wda.launch', 'wda.launch', {
                    ...
                    await launchPreinstalledWda(this.info, connection, client, `${this.info.sessionId}-wda-attempt-${attempt}`, this.wdaFactory, (wda) => {
                        this.wda = wda;
                    });
                });
                await client.ensureReady();
...
export function createDefaultWdaLauncher(info) {
    return new AppiumPreinstalledWdaLauncher(info);
}
function createAppiumPreinstalledWda(info, localPort) {
    const { updatedWDABundleId, updatedWDABundleIdSuffix } = splitWdaBundleId(info.wdaBundleId);
    return new WebDriverAgent({
        device: {
            udid: info.deviceUdid,
            devicectl: {
                launchApp: async (bundleId, opts = {}) => {
...
async function launchPreinstalledWda(info, connection, client, wdaSessionId, wdaFactory, assignWda) {
...
async function waitForWdaReachable(client, timeoutMs) {
    ...
    throw new IOSDeviceBridgeError('wda_launch_failed', `Failed to start the preinstalled WebDriverAgent in ${timeoutMs} ms.`, 502);
}
async function probeWdaStatus(sessionId, client) {
...
function assertPreinstalledSupported(platformVersion) {
    const major = majorVersionFromString(platformVersion);
    if (major !== null && major >= APPIUM_PREINSTALLED_MIN_IOS_MAJOR) {
        return;
    }
    throw new IOSDeviceBridgeError('wda_unsupported_platform', `Preinstalled WDA requires iOS ${APPIUM_PREINSTALLED_MIN_IOS_MAJOR}+ (platform_version=${platformVersion ?? 'unknown'})`, 400, { platform_version: platformVersion });
}
function splitWdaBundleId(wdaBundleId) {
...
function isWdaUiTestingAuthorizationError(error) {
    ...
    const serialized = JSON.stringify(normalizeForDiagnostics({
        message: error.message,
        details: error.details ?? null,
    })).toLowerCase();
    return WDA_UI_TESTING_AUTHORIZATION_PATTERNS.every((pattern) => serialized.includes(pattern));
}
function toWdaLaunchError(info, error, attempt) {
    if (isWdaUiTestingAuthorizationError(error)) {
        return new IOSDeviceBridgeError('wda_ui_testing_not_authorized', 'WDA started, but iOS rejected UI testing actions. Wake and unlock the device, then retry. If needed, launch WDA manually once to establish UI testing authorization.', 502, {
    ...
    if (error instanceof IOSDeviceBridgeError) {
        return new IOSDeviceBridgeError(error.code === 'wda_request_failed' ? 'wda_unreachable' : 'wda_launch_failed', error.message, 502, {
            device_udid: info.deviceUdid,
            platform_version: info.platformVersion,
            wda_bundle_id: info.wdaBundleId,
            attempt_count: attempt,
    ...
    return new IOSDeviceBridgeError('wda_launch_failed', error instanceof Error ? error.message : String(error), 502, {
        device_udid: info.deviceUdid,
        platform_version: info.platformVersion,
        wda_bundle_id: info.wdaBundleId,
        attempt_count: attempt,
...
async function launchWdaRunnerWithOptions(deviceUdid, bundleId, options) {
    const command = [
        'xcrun',
        'devicectl',
        'device',
    ...
    if (options.env && Object.keys(options.env).length > 0) {
        command.push('--environment-variables', JSON.stringify(Object.fromEntries(Object.entries(options.env).map(([key, value]) => [key, String(value)]))));
    }
    command.push(bundleId);
    await execUntilStarted(command);
}
async function execUntilStarted(command, startupWaitMs = 1000) {
