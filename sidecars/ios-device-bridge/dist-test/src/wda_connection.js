...
function defaultConnectionFactory() {
    if (!sharedFactory) {
        sharedFactory = new DeviceConnectionsFactory(createAppiumLogger());
    }
    return sharedFactory;
}
export class WdaConnectionManager {
    createFactory;
    constructor(createFactory = defaultConnectionFactory) {
        this.createFactory = createFactory;
    }
    async requestConnection(sessionId, deviceUdid, platformVersion) {
        ...
        for (const localPort of uniquePorts(candidates)) {
            try {
                const info = await this.requestConnectionOnPort(sessionId, deviceUdid, platformVersion, localPort, factory);
                return info;
            }
            catch (error) {
                lastError = error;
        ...
        throw toConnectionError(deviceUdid, platformVersion, lastError ??
            new Error(`Unable to acquire a free local port for WDA forwarding after ${MAX_DYNAMIC_PORT_ATTEMPTS} attempts`), null);
    }
    async releaseConnection(sessionId, deviceUdid, localPort) {
        await withBridgeSpan(sessionId, 'bridge.wda.connection.release', 'wda.connection', { device_udid: deviceUdid, local_port: localPort }, async () => {
            try {
                await this.createFactory().releaseConnection(deviceUdid, localPort);
            }
            catch (error) {
                throw new IOSDeviceBridgeError('wda_connection_release_failed', error instanceof Error ? error.message : String(error), 502, { device_udid: deviceUdid, local_port: localPort });
            }
        });
    }
    async requestConnectionOnPort(sessionId, deviceUdid, platformVersion, localPort, factory) {
        await withBridgeSpan(sessionId, 'bridge.wda.connection.request', 'wda.connection', {
...
async function buildDynamicLocalPortCandidates() {
    const ports = [];
...
function isPortOccupiedError(error) {
    const message = error instanceof Error ? error.message : String(error);
    const normalized = message.toLowerCase();
    return (normalized.includes('is occupied') ||
        normalized.includes('occupied by an other process') ||
        normalized.includes('occupied by another process'));
}
function toConnectionError(deviceUdid, platformVersion, error, localPort) {
    const message = error instanceof Error ? error.message : String(error);
    const normalized = message.toLowerCase();
    const needsTunnel = normalized.includes('tunnel registry') ||
        normalized.includes('tunnel creation script') ||
        normalized.includes('no tunnel found');
    const portBusy = isPortOccupiedError(error);
    return new IOSDeviceBridgeError(needsTunnel ? 'wda_tunnel_unavailable' : 'wda_connection_failed', message, 502, {
        device_udid: deviceUdid,
        platform_version: platformVersion,
        local_port: localPort,
        device_port: DEFAULT_WDA_REMOTE_PORT,
