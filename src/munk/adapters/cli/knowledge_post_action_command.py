...
def knowledge_post_action_command(
    *,
    request_file: Path,
    ...
    try:
        ...
        resolved_config = require_config_context(
            ...
            command_name="knowledge post action",
        )
    except Exception as exc:
        handle_cli_error(command="knowledge_post_action", exc=exc, json_output=json_output)

    response = MachineCommandService(resolved_config=resolved_config).submit_knowledge_post_action(
        request=request,
        wait=wait,
        detach=detach,
