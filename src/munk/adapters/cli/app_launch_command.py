...
from munk.adapters.shared.machine_requests import AppLaunchRequest
    ...
    *,
    ...
    try:
        ...
        if request_file is not None:
            request = load_json_request(request_file, AppLaunchRequest)
            if assets_root is not None:
                request = request.model_copy(update={"assets_root": assets_root})
        else:
            if app_id is None:
                raise ValueError("app launch requires --app-id")
        ...
        result = AppLifecycleService().launch(request)
        ...
        handle_cli_error(command="app_launch", exc=exc, json_output=json_output)
