...
class OnnxReviewEmbeddingModel:
    """Minimal local ONNX embedding runner for review retrieval."""
    ...
    def __init__(
        ...
        *,
        ...
        if not outputs:
            raise ValueError(f"review embedding ONNX model has no outputs: {model_path}")
    ...
    def encode(self, texts: list[str]) -> np.ndarray:
        ...
        if not model_outputs:
            raise ValueError("review embedding ONNX session returned no outputs")
        ...
        if embeddings.shape[1] != self._embedding_dim:
            raise ValueError(
                f"review embedding dimension mismatch: expected {self._embedding_dim}, got {embeddings.shape[1]}"
            )
    ...
    def _pool(self, last_hidden_state: np.ndarray, attention_mask: np.ndarray) -> np.ndarray:
        if self._pooling_mode != _MEAN_POOLING_KEY:
            raise ValueError(f"unsupported review pooling mode: {self._pooling_mode}")
    ...
    def _load_pooling_mode(model_dir: Path) -> str:
        ...
        if not config_path.exists():
            raise ValueError(f"review embedding pooling config is missing: {config_path}")
        ...
        if truthy_keys != [_MEAN_POOLING_KEY]:
            raise ValueError(
                f"review embedding pooling config must enable only {_MEAN_POOLING_KEY}: {config_path}"
            )
