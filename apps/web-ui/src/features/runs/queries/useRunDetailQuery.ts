...
import { getOperation } from '@/shared/api/operations'
...
export function useRunDetailQuery(operationId: MaybeRefOrGetter<string>) {
  return useQuery({
    queryKey: computed(() => runsKeys.detail(toValue(operationId))),
    queryFn: () => getOperation(toValue(operationId)),
