...
`review-api` 是 `ai-monkey` review 能力的共享 contract 包。
...
当前主仓通过 `review-api` 暴露的 runtime 协议发现并调用已安装的 review runtime，具体入口见：
...
- `review-runtime-local` 依赖 `review-api`
...
- 不要把 runtime 内部实现拉进这里
  - `review-api` 只负责“长什么样”，不负责“怎么做出来”

## 变更 checklist

修改 `review-api` 前，至少确认：
